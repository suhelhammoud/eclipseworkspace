...
//		assert simotree != null;
		...
		while (Sim_system.running()) {
			...
			if(tag == HTAG.simotree_add_rack.id()){
				simotree.addRack("rack_1", (Map<String, HMachine>)ev.get_data());
				logger.info("add rack "+ 
						((Map<String, HMachine>)ev.get_data()).size());
				continue;
			}
...
//				logger.debug(ev.get_data());
			...
			if(tag == HTAG.simotree_moveup_reduce.id()){
				simotree.moveUpReducerStory((ReduceCopier) ev.get_data());
				continue;
			}
