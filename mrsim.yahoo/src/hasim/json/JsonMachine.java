...
public class JsonMachine {
	...
	private JsonHardDisk hardDisk=new JsonHardDisk();
	private JsonCpu cpu=new JsonCpu();
	...
	public void setHostName(int hostName) {
		assert hostName < JsonRealRack.MAX_MACHINE_IN_RACK
			&& hostName > Datum.LOCATION_NONE ;
	...
	public JsonMachine copy(int name){
		...
		result.maxMapper=maxMapper;
		result.maxReducer=maxReducer;
		result.mapperVirtualMemory=mapperVirtualMemory;
		result.reducerVirtualMemory=reducerVirtualMemory;
		result.virtualMemory=virtualMemory;
